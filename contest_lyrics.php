...
$lyricsList = [];
$lyricWinner = null;
$userSubmissionCount = 0; // Счётчик поданных текстов

try {
    // Ищем самый последний конкурс, который еще не полностью закрыт
    ...
    if ($activeContest) {
        if ($activeContest['status'] === 'submission_active' || $activeContest['status'] === 'voting_active') {
            ...
            // Считаем, сколько текстов подал текущий пользователь
...
// Функция для отображения списка текстов
function renderLyricsList($lyrics, $currentUser) {
    if (empty($lyrics)) {
        ...
        foreach($lyrics as $lyric) {
            ...
            echo '</div>';
            echo '<p class="lyric-author">Автор: ' . htmlspecialchars($lyric['author']) . '</p>';
            ...
            echo '<button type="button" class="vote-button lyric-vote-button" title="Голосовать за текст">❤</button>';
            echo '</div>';
            echo '</div>';
            echo '</form>';
            echo '</div>';
...
<h1>Конкурс текстов <?php if ($activeContest) echo ' - ' . htmlspecialchars($activeContest['name']); ?></h1>
...
if (isset($_GET['error']) && $_GET['error'] === 'limit_reached') echo '<div class="alert error">Вы уже добавили максимальное количество текстов (2) в этот конкурс.</div>';
    ...
    switch ($activeContest['status']) {
        ...
        case 'submission_active':
            ...
            <?php
                ...
                echo '<div class="alert">Вы достигли лимита (2 текста) в этом конкурсе.</div>';
            }
            
            echo '<div class="lyrics-list"><h2>Уже на конкурсе</h2>';
            renderLyricsList($lyricsList, $currentUser);
            echo '</div>';
            break;
